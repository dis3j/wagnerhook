...
float GetBulletSpeed() {
	...
	return vars::weapons::fast_bullets ? 350.f * 1.3 + vars::stuff::testFloat : 350.f + vars::stuff::testFloat;
}
...
void do_aimbot(BasePlayer* player, float bullet_travel_time) {
	...
	Vector3 newVelocity = player->newVelocity();
	float gravity;
	...
	Vector3 bulletEndPos = a::SimulateProjectile(local, newVelocity, bullet_travel_time, bullet_travel_time, gravity);
	Vector2 AngleToAim = Math::CalcAngle(local, bulletEndPos) - LocalPlayer::Entity()->viewangles();
	Math::Normalize(AngleToAim.y, AngleToAim.x);
	LocalPlayer::Entity()->set_viewangles(LocalPlayer::Entity()->viewangles() + AngleToAim);
	if (vars::combat::smooth) {
		StepConstant(LocalPlayer::Entity()->viewangles() + AngleToAim);
	}
}